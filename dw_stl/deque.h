#ifndef DW_STL_DEQUE_H_
#define DW_STL_DEQUE_H_

/*
这个头文件包含了一个模板类 deque

异常保证：
dw_stl::deque<T> 满足基本异常保证，部分函数无异常保证，并对// 需要分配的缓冲区个数以下等函数做强异常安全保证：
   * emplace_front
   * emplace_back
   * emplace
   * push_front
...
namespace dw_stl
{
    ...
    // deque的迭代器
    // deque迭代器使用random_access_iterator_tag
    ...
    public:
        ...
        template <class InputIter, typename std::enable_if<dw_stl::is_input_iterator<InputIter>::value, int>::type = 0>
        ...
        { 
            copy_assign(ilist.begin(), ilist.end(), dw_stl::forward_iterator_tag{}); 
        }
    ...
    if (this != &rhs)
    {
        ...
        if (len >= rhs.size())
        {
            erase(dw_stl::copy(rhs.begin_, rhs.end_, begin_), end_);
        }
        else
        {
    ...
    // 在头部就地构建元素(emplace_front, 减少一次拷贝构造)
    template <class T>
    template <class ...Args>
    ...
    typename deque<T>::iterator deque<T>::erase(iterator position)
    {
        ...
        {
            dw_stl::copy_backward(begin_, position, next);
            pop_front();
        }
        else
        {
    ...
    void deque<T>::map_init(size_type nElem)
    {
        ...
        map_size_ = dw_stl::max(static_cast<size_type>(DEQUE_MAP_INIT_SIZE), nNode + 2);
        try
        {
    ...
    template <class InputIter>
    void deque<T>::
    copy_init(InputIter first, InputIter last, input_iterator_tag)
    {
    ...
    {  
        ...
        {
            erase(dw_stl::copy(first, last, begin_), end_);
        }
    }
    ...
    // fill_insert函数, 在positon处填入n个元素value
    template <class T>
    void deque<T>::fill_insert(iterator position, size_type n, const value_type& value)
    {
        ...
        if (elems_before < (len / 2))
        {
            ...
            try
            {
                ...
                {
                    dw_stl::uninitialized_fill(
                    dw_stl::uninitialized_copy(begin_, position, new_begin), begin_, value_copy);
                    begin_ = new_begin;
    ...
    // copy_insert, 在位置position处复制其他迭代器的元素
    template <class T>
    template <class ForwardIter>
        ...
        if (elems_before < (len / 2))
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    dw_stl::uninitialized_copy(first, mid,
                                            dw_stl::uninitialized_copy(begin_, position, new_begin));
                    begin_ = new_begin;
        ...
        else
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    dw_stl::uninitialized_copy(position, end_,
                                            dw_stl::uninitialized_copy(mid, last, end_));
                    end_ = new_end;
    ...
    // reallocate_map_at_front函数，在头部扩充内存
    template <class T>
    void deque<T>::reallocate_map_at_front(size_type need_buffer)
    {
        const size_type new_map_size = dw_stl::max(map_size_ << 1,
    ...
    // reallocate_map_at_back函数，在尾部扩充内存
    template <class T>
    ...
    bool operator<(const deque<T>& lhs, const deque<T>& rhs)
    {
        return dw_stl::lexicographical_compare(
